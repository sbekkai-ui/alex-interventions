...
const STORAGE_KEY = "interventions-hub-data";

function loadInitial(): Intervention[] {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (raw) return JSON.parse(raw).map((i: Intervention) => ({ location: "", ...i }));
  } catch {}
  return seedInterventions;
}
...
export default function Dashboard() {
  const [interventions, setInterventions] = useState<Intervention[]>(loadInitial);
  ...
  // Persist to localStorage so data survives between desktop app launches
  useMemo(() => {
    try {
  ...
  const pendingEngineers = useMemo(() => {
    ...
    interventions.filter(i => i.reportStatus === "En attente").forEach(i => counts.set(i.engineer || "Sans ingénieur", (counts.get(i.engineer || "Sans ingénieur") || 0) + 1));
  ...
  function isInSelectedPeriod(dateValue: string, period: string, referenceValue: string) {
    ...
    if (period === "day") return date.toDateString() === reference.toDateString();
    if (period === "week") {
  ...
  const deleteTargets = useMemo(() => interventions.filter(i => isInSelectedPeriod(i.plannedDate, deletePeriod, latestPlannedDate(interventions))), [interventions, deletePeriod]);

  function nextNumber(prev: Intervention[]) {
    const year = new Date().getFullYear();
    const prefix = `INT-${year}-`;
    const max = prev.map((p) => p.number).filter((n) => n.startsWith(prefix)).map((n) => parseInt(n.slice(prefix.length), 10)).filter((n) => !isNaN(n)).reduce((a, b) => Math.max(a, b), 0);
    return `${prefix}${String(max + 1).padStart(3, "0")}`;
  }

  function openCreate() { setEditing({ ...emptyDraft(), number: nextNumber(interventions) }); setFormOpen(true); }
  function openEdit(row: Intervention) { setEditing(row); setFormOpen(true); }
  ...
  function handleDeletePeriod() {
    if (deleteTargets.length === 0) return toast.info("Aucune intervention à supprimer");
    const ok = window.confirm(`Supprimer ${deleteTargets.length} intervention${deleteTargets.length > 1 ? "s" : ""} ?`);
    if (!ok) return;
        ...
        <div className="mx-auto flex max-w-[1400px] items-center gap-4 px-6 py-3.5">
          <div className="flex items-center gap-2.5">
            ...
            <div className="hidden sm:block">
              <h1 className="text-base font-semibold tracking-tight">InterventionsHub</h1>
          ...
          <div className="relative ml-auto md:ml-6 md:max-w-md flex-1">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher N°, client, ingénieur…" className="pl-9 h-10 bg-muted/40 border-border/60 focus-visible:bg-background" />
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" className="hidden md:inline-flex gap-1.5" onClick={() => toast.success("Export en cours…")}>
              <Download className="h-4 w-4" /> Export
            </Button>
            <Button size="sm" className="gap-2 bg-[var(--gradient-primary)] hover:opacity-90 shadow-[var(--shadow-elegant)] rounded-full px-4 font-semibold" onClick={openCreate}>
      ...
      <main className="mx-auto max-w-[1400px] px-6 py-8 space-y-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Pilotez vos interventions techniques en temps réel.</p>
          </div>
          <Button size="lg" onClick={openCreate} className="gap-2 bg-[var(--gradient-primary)] hover:opacity-90 shadow-[var(--shadow-elegant)] rounded-xl font-semibold transition-transform hover:scale-[1.02]">
          ...
          <StatCard
            ...
            hoverContent={
              <div className="space-y-2">
                ...
                {pendingEngineers.length === 0 ? <p className="text-sm text-muted-foreground">Aucun rapport en attente</p> : (
                  <ul className="space-y-1.5">
                    {pendingEngineers.map(([name, count]) => <li key={name} className="flex items-center justify-between gap-4 text-sm"><span>{name}</span><span className="font-semibold text-foreground">{count}</span></li>)}
                  </ul>
                )}
              </div>
            }
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant="destructive" size="sm" onClick={handleDeletePeriod} disabled={deleteTargets.length === 0} className="gap-1.5">
        ...
        <InterventionsTable data={filtered} onEdit={openEdit} onComplete={handleComplete} onDelete={handleDelete} reportOptions={reportOptions} fileOptions={fileOptions} onReportChange={(row, v) => setInterventions((prev) => prev.map((i) => i.id === row.id ? { ...i, reportStatus: v } : i))} onFileChange={(row, v) => setInterventions((prev) => prev.map((i) => i.id === row.id ? { ...i, fileStatus: v } : i))} />
      </main>

      <InterventionForm open={formOpen} onOpenChange={setFormOpen} initial={editing} reportOptions={reportOptions} fileOptions={fileOptions} onSubmit={handleSubmit} />
    </div>
  );
}
